...
final class String{
  ...
  public static function findClosingTag( $sourceString, $tagName, $tagStart, $tagEnd ){

  }

  public static function parseTagAttributes( $tag ){
    $attr = array();
    preg_match_all('/([\w\-]+)(?:\s*\=\s*([\"\'])(.*?)\\2)?/', $tag, $match);
    foreach( $match[1] as $i => $prop ){
      $attr[$prop] = $match[3][$i];
    }
    return $attr;
  }
